    ...
    return (
        <aside className={`w-20 lg:w-72 border-r border-slate-800 flex flex-col bg-[#0b0f1a] z-20 shadow-2xl transition-all duration-500 transform-gpu will-change-[filter,transform,opacity] ${showSOS ? 'blur-md grayscale opacity-50 scale-95 pointer-events-none' : ''}`}>
            <div className="p-8 flex items-center gap-3">
                ...
                <span className="hidden lg:block text-2xl font-black text-white italic uppercase tracking-tighter">Shelter</span>
            </div>

            <nav className="flex-1 px-4 mt-6">
                <div className="space-y-0">
                    <FlipSidebarItem id="home" icon={<Layout size={22} />} label="Дашборд" isDashboard={true} index={0} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} onBackAction={handleChatBack} />
                    <FlipSidebarItem id="quests" icon={<Trophy size={22} />} label="Квести" index={1} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} />
                    <FlipSidebarItem id="testing" icon={<ClipboardList size={22} />} label="Діагностика" index={2} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} />
                    <FlipSidebarItem id="library" icon={<BookOpen size={22} />} label="Медіатека" index={3} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} />
                    <FlipSidebarItem id="advice" icon={<Lightbulb size={22} />} label="Поради" index={4} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} />
                    <FlipSidebarItem id="diary" icon={<PenLine size={22} />} label="Щоденник" index={5} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} />
                    <FlipSidebarItem id="stats" icon={<BarChart3 size={22} />} label="Прогрес" index={6} isSpecialMode={isSpecialMode} currentView={currentView} onClickAction={navigateTo} />
                </div>
            </nav>
