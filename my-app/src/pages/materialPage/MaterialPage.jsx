...
export default function MaterialPage() {
    ...
    const handleComplete = async (delta = 5) => {
        ...
        const viewTime = Math.round((Date.now() - viewStartTime) / 60000);
        if (userId && viewTime > 0) {
            api.recordMaterialView(userId, id, viewTime).catch(err => console.error(err));
        }
        navigate(-1);
    };
        ...
        const fetchData = async () => {
            try {
                ...
                if (isMounted && materialData) {
                    ...
                    if (userId) {
                        api.recordMaterialView(userId, id).catch(err => console.error(err));
                    }
                }
                                ...
                                {!showFeedback ? (
                                    ...
                                    <div className="bg-slate-900/40 border border-slate-800 rounded-[40px] p-8 backdrop-blur-xl shadow-2xl text-center">
                                        ...
                                        <div className="flex gap-4 justify-center">
                                            <button onClick={() => handleComplete(5)} className="bg-emerald-500 hover:bg-emerald-400 text-[#0b0f1a] px-8 py-3 rounded-2xl font-black uppercase text-xs shadow-xl transition-all">😊 Покращився</button>
                                            <button onClick={() => handleComplete(2)} className="bg-slate-700 hover:bg-slate-600 text-white px-8 py-3 rounded-2xl font-black uppercase text-xs transition-all">😐 Без змін</button>
                                        </div>
                                    </div>
                                )}
                            </section>
